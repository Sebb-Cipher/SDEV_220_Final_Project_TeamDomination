...
class FleetManagementSystem:
    ...
    def add_call_schedule(self, call_schedule):
        # Add a new call schedule to the call_schedules table
        cursor = self.conn.cursor()
        cursor.execute('''
...
class FleetManagementApp(tk.Tk):
    ...
    def create_dashboard_tab(self):
        ...
        self.schedule_tree_dashboard = ttk.Treeview(schedule_frame, columns=("ID", "Customer Name", "Date", "Time", "Vehicle ID"), show="headings")
    ...
    def add_call_schedule_popup(self):
        ...
        selected_kit = tk.StringVar()
        kits = list(self.fleet_system.inventory.items.keys())

        def select_kit():
            kit_popup = tk.Toplevel(popup)
            kit_popup.title("Select Service Kit")

            for i, kit in enumerate(kits):
                tk.Radiobutton(kit_popup, text=kit, variable=selected_kit, value=kit).grid(row=i, column=0, sticky="w", padx=10, pady=5)

            def confirm_kit():
                if selected_kit.get():
                    kit_popup.destroy()
                else:
                    messagebox.showwarning("No Kit Selected", "Please select a service kit.")

            tk.Button(kit_popup, text="Confirm", command=confirm_kit).grid(row=len(kits), column=0, pady=10)

        tk.Button(popup, text="Select Service Kit", command=select_kit).grid(row=4, column=0, columnspan=2, pady=10)

        def add_call_schedule():
            if not selected_kit.get():
                messagebox.showwarning("No Kit Selected", "Please select a service kit before scheduling the call.")
                return

            if self.fleet_system.inventory.use_item(selected_kit.get()):
                call_schedule = CallSchedule(call_id_entry.get(), customer_name_entry.get(), date_entry.get_date().strftime("%Y-%m-%d"), time_entry.get())
                self.fleet_system.add_call_schedule(call_schedule)
                self.refresh_schedule_list()
                popup.destroy()
            else:
                messagebox.showwarning("Inventory Error", f"No {selected_kit.get()} available in inventory.")
    ...
    def assign_vehicle_popup(self):
        ...
        if selected_item:
            ...
            def assign_vehicle():
                if self.inventory_checklist():
                    self.fleet_system.assign_vehicle_to_call(call_id, vehicle_id_entry.get())
                    self.refresh_schedule_list()
                    self.refresh_vehicle_list()
                    popup.destroy()
                else:
                    messagebox.showwarning("Inventory Check Failed", "Please complete the inventory checklist before assigning a vehicle.")
    ...
    def inventory_checklist(self):
        ...
        for i, item in enumerate(items):
            var = tk.BooleanVar()
            ttk.Checkbutton(checklist_popup, text=item, variable=var).grid(row=i, column=0, sticky="w", padx=10, pady=5)
            var_dict[item] = var
        ...
        def confirm_checklist():
            if all(var.get() for var in var_dict.values()):
                ...
                messagebox.showwarning("Incomplete Checklist", "Please check all items before confirming.")
                return False

        ttk.Button(checklist_popup, text="Confirm", command=confirm_checklist).grid(row=len(items), column=0, pady=10)

        self.root.wait_window(checklist_popup)
        return all(var.get() for var in var_dict.values())
